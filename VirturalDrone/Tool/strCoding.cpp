...
void strCoding::Gb2312ToUnicode(WCHAR* pOut,char *gbBuffer)  
{  
	//size_t nlen = mbstowcs(NULL, (const char *)gbBuffer, 0);
	mbstowcs(pOut, (const char *)gbBuffer, 1/*nlen*/);
	//printf("Gb2312ToUnicode nlen = %d\n", nlen);
	return;  
}  

void strCoding::UnicodeToGB2312(char* pOut,WCHAR uData)  
{  
	size_t nlen = wcstombs(pOut,NULL,0);
	wcstombs(pOut,(const wchar_t *)uData,nlen*sizeof(WCHAR));
	printf("UnicodeToGB2312 nlen = %d\n", nlen);
	return;  
}  
#endif
...
string strCoding::UrlUTF8Encode(char * str)
{
	string tt = str;
	string dd;  
