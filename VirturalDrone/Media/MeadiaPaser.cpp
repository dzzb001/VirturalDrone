...
int CMeadiaPaser::InitFFmpeg()
{
    ...
    //av_dict_set(&options, "rtsp_transport", "udp", 0); // ����udp����¼��ʱ��ʱ��pause �ᵼ���޷��ָ�������
    // ���ô���Э��ΪTCPЭ��
    ...
    if (!m_AVFormatContext)
    {
        ...
        //m_AVFormatContext->flags |= AVFMT_FLAG_NONBLOCK; //���óɷ�����
        m_AVFormatContext->interrupt_callback.opaque = (void*)this;
        m_AVFormatContext->interrupt_callback.callback = interruptCallback;//���ûص�

        int pos = m_strUrl.rfind(".");
        if (pos >= 0)
            m_strSuffix = m_strUrl.substr(pos);
        if (!m_strSuffix.compare(".flv"))
            m_AVFormatContext->probesize = 32; //add at 20230625 ,������ʽ����Ҫ̽�⣬�����п��ܵ�����Ƶ�޷�����
        ...
        if (m_AVFormatContext->nb_streams == 0) //Ϊ0����̽�⣬������Щ��Ƶ�޷����ţ�����Ĳ�Ϊ0
        {
            {
        ...
        if (codecpar->codec_id != AV_CODEC_ID_NONE)//δʶ��AV_CODEC_ID������Ҫ���ô˽ӿڣ�����˽ӿڻ�һֱ������̽����ֱ��7��8ֹͣ�������¿����ӳ�����
        {
            if (!bFindStream) //�Ѿ�̽��������findstream
                ret = avformat_find_stream_info(m_AVFormatContext, nullptr);
        }
...
void CMeadiaPaser::Es2RtpThread()
{
    while (true)
    {
...

        {
            ...
            //AVPacket ������ʽ������
            //1��������ԭʼ���ݱ���õ���AVPacket��ʽ���׼��ʽ��ͬ������ʼ��startcode+ʵ������
            //2�����ڽ��װ�õ���AVPacket��ʽΪ���ֽڳ���+ʵ������ ��ǰ�ĸ��ֽڱ�ʾʵ�����ݳ��� 
            // ���ǽ� �ڶ��ָ�ʽת���ɵ�һ�ָ�ʽ��
#if FILTER
            {
                int input_size = pkt->size;
                ...
                while (av_bsf_receive_packet(m_bsf_ctx, pkt) == 0)
                {
                    ...
                    UINT uPts = uTimestamp * 90000 / 1000;  //RTP时间戳需要90K的频率
            ...
            UINT uPts = uTimestamp * 90000 / 1000;  //RTP��ʱ�������90K������
            m_es2Rtp.EsIn(pkt->data, pkt->size, uPts);
...
bool CMeadiaPaser::probe_stream_type(const AVPacket* pkt, std::string& strType)
{
    ...
    while (offset < size - 3) {
        ...
        if ((data[offset] == 0x00 && data[offset + 1] == 0x00 && data[offset + 2] == 0x01) ||
            ...
            nal_type_hevc = (data[nal_start] >> 1) & 0x3F; //hevc nal head 2���ֽ� ��ȡ NAL ͷ ��һ���ֽڵĸ� 6 λ
