...
typedef struct AVInputFormat {
    ...
    int (*read_pause)(struct AVFormatContext *);
	 ...
	 /**
     * Playing with specific speed - only meaningful if using a network-based format
     * (RTSP).
     */
	int (*read_scale)(struct AVFormatContext *,float);
...
/**
 * Playing a network-based stream (e.g. RTSP stream) with specified speed.
 * add by bzhang
 */
int av_read_scale(AVFormatContext *s,float scale);
