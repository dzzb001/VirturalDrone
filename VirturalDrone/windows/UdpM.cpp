...
* 2017/04/12	张斌	      创建
*******************************************************************************/
bool CUdpM::Start( int nRecvBuf /* = 0 */, int nSendBuf /* = 0 */, bool bCheckConnect /*= false*/, unsigned short nPort /* = 0 */,
...
void CUdpM::ExeSend(const char *pData, int nLen, const std::string &strToIP /* = "" */, unsigned short nToPort /* = 0 */)
{
	if (!strToIP.empty())
	{
		...
		if (1 != inet_pton(AF_INET, strToIP.c_str(), (void*)&m_pContextSend->addSendTo.sin_addr))
		{
			Log("[CUdpM::ExeSend] 发送的目的地址<%s>格式或信息有误！", strToIP.c_str());
			return;
		}
...
* 2017/04/11	张斌	      创建
*******************************************************************************/
bool CUdpM::ErrCB(std::shared_ptr<void> pContext, SOCKET s, int nErrCode)
{
	...
	if (nErrCode == 1234 && !pThis->m_bCheckConnect)
	{
		//发送的端口不可达时，RecvFrom会返回这个错误
		return false;
	}
	pThis->OnErr();
	pThis->Clear();
...
bool CUdpM::RecvFromCB(std::shared_ptr<void> pContext, SOCKET s, char *pData, int nLen, const std::string strFromIP, unsigned short nFromPort)
{
	...
	if (strFromIP == pThis->m_strLocalIP && nFromPort == pThis->m_nLocalPort && nLen>0)
	{
		//尝试清理一次
...
void CUdpM::CloseSocket(SOCKET &s)
{
	if (INVALID_SOCKET != s)
	{
		if (0 != closesocket(s))
		{
			Log("[CUdpM::CloseSocket] 关闭socket失败！-- <%d>", WSAGetLastError());
		}
		s = INVALID_SOCKET;
	}
}
