	...
	{
		{
			...
			nTimer = m_setTimer.begin()->get()->nFlag;
		}
		DelTimer(nTimer);
	}
	m_bStop = true;
	...
	while (!m_bStop)
	{
		...
		if (!bRet)
		{
			...
			{     
				...
				bDelete = OnErrDelete(pContext);
				//OutputDebugString(std::to_string(bDelete).c_str());
				//OutputDebugString("\n");
			}
			if (bDelete)
			{
		...
		Context *pContext = CONTAINING_RECORD(lpOverlapped, Context, ovlp); //投递的时候传入的
		bool bExeRet = false;
...
bool CMonitor::ExeRecvFrom(Context *pContextBase, DWORD nDataLen)
{
	...
	if (nullptr != pContext->pfnRecv)
	{
		...
		/*char buf[256] = { 0 };
		sprintf(buf,"[%s]Socket<%d>接收到来自<%s:%d>的：%d 字节\n", __FUNCTION__, pContext->s,
...
//发生错误的处理，删除环境变量
bool CMonitor::OnErrDelete(Context *pContext)
{
	bool bDelete = true;
	if (nullptr != pContext->pfnErr)
	{
		bDelete = pContext->pfnErr(pContext->pContextErr, pContext->s, WSAGetLastError());
	}
	if (bDelete)
	{
		delete pContext;
