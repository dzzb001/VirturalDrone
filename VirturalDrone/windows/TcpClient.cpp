...
#define RCV_BUF_SIZE 1024*1024*8
	...
	int nSendBuf /* = 0 */, const std::string &strLocalIP /* = "" */, unsigned short nLocalPort /* = 0 */ )
{
	...
	OutputDebugString(m_strFlag.c_str());
	return true;
}
...
* 2017/03/07	张斌	      创建
*******************************************************************************/
bool CTcpClient::ErrCB(std::shared_ptr<void> pContext, SOCKET s, int nErrCode)
{
	CTcpClient *pThis = (CTcpClient*)pContext.get();
	if (pThis == nullptr)
	{
