...
#define DEBUG_NEW_HASH(p) ( ((ULONG)(p) >> 8) % DEBUG_NEW_HASHTABLESIZE)
#endif
    ...
    while (ptr)
    {
        if ((char*)ptr + sizeof(new_ptr_list_t) == pointer)
        {
            if (array != ptr->array)
            {
                if (ptr->array)
                {
                    fprintf(stderr, "delete: you must call delete [] %p, %p allocated by %s:%d\n", 
                                    pointer, pointer, 
                                    ptr->file, ptr->line);
